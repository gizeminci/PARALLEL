...
/** send buffer*/
int n_s_buffer_bond=0;
static int max_s_buffer_bond = 0;
static char *s_buffer_bond = NULL;
/** recv buffer*/
static int n_r_buffer_bond = 0;
static int max_r_buffer_bond = 0;
static char *r_buffer_bond = NULL;
...
int calc_transmit_size(GhostCommunication *gc, int data_parts)
{
  ...
  if (data_parts == GHOSTTRANS_PARTNUM)
    ...
    // contains number of bonds to send
    if (data_parts & GHOSTTRANS_PROPRTS)
      n_buffer_new += sizeof(int);  
...
void prepare_send_buffer(GhostCommunication *gc, int data_parts)
{
  ...
  // reallocate send buffer bond [alloc_intlist(&pt->bl, pt->bl.n);]
for (pl = 0; pl < gc->n_part_lists; pl++) {
  np   = gc->part_lists[pl]->n;
  part = gc->part_lists[pl]->part;
  for (p = 0; p < np; p++) {
      pt = &part[p];
      n_s_buffer_bond += pt->bl.n*sizeof(int);
   }
//printf("n_s_buffer_bond size is:%d\n",n_s_buffer_bond);
}
  if (n_s_buffer_bond > max_s_buffer_bond) {
     max_s_buffer_bond = n_s_buffer_bond;
     s_buffer_bond = realloc(s_buffer_bond, max_s_buffer_bond);
    }
  GHOST_TRACE(fprintf(stderr, "%d: will send %d\n", this_node, n_s_buffer_bond));

  /* put in data */
  char *insert = s_buffer;
  char *insert_bond = s_buffer_bond;
  for (pl = 0; pl < gc->n_part_lists; pl++) {
    np   = gc->part_lists[pl]->n;
	...
	if (data_parts & GHOSTTRANS_PROPRTS) {
	  ...
	  insert +=  sizeof(ParticleProperties);
          ...
          int i = 0;
          while(i < pt->bl.n) {
           size = bonded_ia_params[pt->bl.e[i]].num;
           memcpy(insert_bond, pt->bl.e, pt->bl.n*sizeof(int));
           insert_bond +=  pt->bl.n*sizeof(int);
           i += size+1;
  	  }         
	}
  ...
  if (data_parts & GHOSTTRANS_PROPRTS) {
    memcpy(insert, &n_s_buffer_bond, sizeof(int));
    insert += sizeof(int);
  }

#ifdef ADDITIONAL_CHECKS
  if (insert_bond - s_buffer_bond != n_s_buffer_bond) {
    fprintf(stderr, "%d: INTERNAL ERROR: send buffer bond size %d differs from what I put in %d\n", this_node, n_s_buffer_bond, insert_bond - s_buffer_bond);
    errexit();
  }
...
void prepare_bond_recv_buffer(int bond)
{
n_r_buffer_bond = bond;

  if (n_r_buffer_bond > max_r_buffer_bond) {
    max_r_buffer_bond = n_r_buffer_bond;
    r_buffer_bond = realloc(r_buffer_bond, max_r_buffer_bond);
  }
  GHOST_TRACE(fprintf(stderr, "%d: will get %d\n", this_node, n_r_buffer_bond));
...
void put_recv_buffer(GhostCommunication *gc, int data_parts)
{
  ...
  char *retrieve_bond = r_buffer_bond;
  for (pl = 0; pl < gc->n_part_lists; pl++) {
    if (data_parts == GHOSTTRANS_PARTNUM) {
	...
	if (data_parts & GHOSTTRANS_PROPRTS) {
	  ...
	  retrieve +=  sizeof(ParticleProperties);
 	  memcpy(pt->bl.e, retrieve_bond, pt->bl.n*sizeof(int));
	  retrieve_bond +=  pt->bl.n*sizeof(int);
  ...
  if (data_parts & GHOSTTRANS_PROPRTS) {
    memcpy(retrieve, &n_r_buffer_bond, sizeof(int));
    retrieve += sizeof(int);
   }
  ...
  if (retrieve_bond - r_buffer_bond != n_r_buffer_bond) {
    fprintf(stderr, "%d: recv buffer bond size %d differs from what I put in %d\n", this_node, n_r_buffer_bond, retrieve_bond - r_buffer_bond);
    errexit();
  }
#endif
}
  ...
  for (pl = 0; pl < offset; pl++) {
	...
	if (data_parts & GHOSTTRANS_PROPRTS){
	  memcpy(&pt2->p, &pt1->p, sizeof(ParticleProperties));
          memcpy(&pt2->bl, &pt1->bl, sizeof(IntList));
	}
	if (data_parts & GHOSTTRANS_POSSHFTD) {
	  /* ok, this is not nice, but perhaps fast */
	  int i;
...
void ghost_communicator(GhostCommunicator *gc)
{
  ...
  for (n = 0; n < gc->num; n++) {
	...
	// receive bonds. No buffer preparing possible
	if (data_parts & GHOSTTRANS_PROPRTS) {
        int bonds = *(int *)(r_buffer + n_r_buffer - sizeof(int));
	prepare_bond_recv_buffer(bonds);
	MPI_Recv(r_buffer_bond, n_r_buffer_bond, MPI_BYTE, node, REQ_GHOST_SEND, comm_cart, &status);
	}
        break;
      case GHOST_SEND:
	...
	MPI_Send(s_buffer_bond, n_s_buffer_bond, MPI_BYTE, node, REQ_GHOST_SEND, comm_cart);
	break;
      case GHOST_BCST:
	...
	if (node == this_node) {
	  ...
	  MPI_Bcast(s_buffer_bond, n_s_buffer_bond, MPI_BYTE, node, comm_cart);
        }
	else
	...
	break;
      ...
      /* Here, should the number of bonds or the total amount of bytes be controlled ?? */

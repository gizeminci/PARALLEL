  ...
  */
  for (c = 0; c < local_cells.n; c++) {
    ...
    for (i = 0; i < np; i++) {
      ...
      //printf("0. Particle(REAL) %d has a bond %d\n", p[i].p.identity, p[i].bl.n);

part1 = p[i].p.identity;

ni = 0;
  while(ni < p[i].bl.n) {
    size = bonded_ia_params[p[i].bl.e[ni]].num;
printf("REAL PARTICLE %d SIZE OF THE BONDS %d Bond Type %d and Bonded partner %d AT THIS NODE: %d\n", part1, size, p[i].bl.e[ni], p[i].bl.e[ni + 1], this_node);
fflush(stdout);
    ni += size + 1;
  }
  ...
  */
  for (c = 0; c < ghost_cells.n; c++) {
    ...
    for (i = 0; i < np; i++) {
     ...
     // printf("1. Particle(GHOST) %d has a bond %d\n", p[i].p.identity, p[i].bl.n);
...
ni = 0;
  while(ni < p[i].bl.n) {
    size = bonded_ia_params[p[i].bl.e[ni]].num;
printf("GHOST PARTICLE %d SIZE OF THE BONDS %d Bond Type %d and Bonded partner %d AT THIS NODE: %d\n", part1, size, p[i].bl.e[ni], p[i].bl.e[ni + 1], this_node);
fflush(stdout);
    ni += size + 1;
  }
...
void init_forces_ghosts()
{
  ...
  for (c = 0; c < ghost_cells.n; c++) {
    ...
    for (i = 0; i < np; i++) {
     ...
     // printf("2. Particle(GHOST) %d has a bond %d\n", p[i].p.identity, p[i].bl.n);

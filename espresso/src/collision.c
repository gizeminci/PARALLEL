...
printf("COLLIDED PARTICLES ARE %d and %d and GHOST CONTROL %d and %d\n", p1->p.identity, p2->p.identity, p1->l.ghost, p2->l.ghost);

  //printf("Collided particles are %d and %d\n", p1->p.identity, p2->p.identity);
  part1 = p1->p.identity;
  part2 = p2->p.identity;
  ...
  printf("Check-1, if there's already a bond between the particles %d and %d, BONDS: %d and %d\n", p1->p.identity, p2->p.identity, p1->bl.n, p2->bl.n);
  // First check the bonds of p1 
  int i = 0;
  while(i < p1->bl.n) {
    size = bonded_ia_params[p1->bl.e[i]].num;

    if (p1->bl.e[i] == collision_params.bond_centers && p1->bl.e[i + 1] == part2) {
     // printf("Particle(p1) %d has a bond of type %d - (%d) with Particle %d - (%d), already. Nothing to do for these particles\n", part1, p1->bl.e[i], collision_params.bond_centers, p1->bl.e[i + 1], part2);
      return;
    }
    i += size + 1;
  }     
  printf("NO BOND STORED IN P-%d -- GHOST-CHECK %d\n", p1->p.identity, p1->l.ghost);
  //printf("\n");

  printf("Check-3, if a bond is already stored in p-%d BONDS: %d\n", p2->p.identity, p2->bl.n);
  ...
  while(i < p2->bl.n) {
    ...
    if (p2->bl.e[i] == collision_params.bond_centers && p2->bl.e[i + 1] == part1) {
      //printf("Particle(p2) %d has a bond of type %d - (%d) with Particle %d - (%d), already. Nothing to do for these particles\n", part2, p2->bl.e[i], collision_params.bond_centers, p2->bl.e[i + 1], part1);
      return;
    }
    i += size + 1;
  }

  printf("If we're still here, there is no previous bond between the particles, we have a new collision\n");

#ifdef GHOST_FLAG
printf("check the particles are ghost or not, if not create bond\n");
  /* create marking bond between the colliding particles immediately */
  if (collision_params.mode & COLLISION_MODE_BOND) {
    int bondG[2];
printf("GHOST CHECK-1 --> Particle %d is %d\n", p1->p.identity, p1->l.ghost);
    if (p1->l.ghost==0) {
      bondG[0]=collision_params.bond_centers;
      bondG[1]=part2;
      local_change_bond(part1, bondG, 0);
      printf("bond is created on Particle-%d, Type %d, With Particle %d\n", part1, p1->bl.e[0], p1->bl.e[1]);
      printf("\n");
     } 
    else if (p2->l.ghost==0) {
printf("GHOST CHECK-2 --> Particle %d is %d\n", p2->p.identity, p2->l.ghost);
      bondG[0]=collision_params.bond_centers;
      bondG[1]=part1;
      local_change_bond(part2, bondG, 0);
      printf("bond is created on Particle-%d, Type %d, With Particle %dd\n", part2, p2->bl.e[0], p2->bl.e[1]);
      printf("\n");
     }
    else {
      printf("Particles are ghost particles");
        return;
    }
  }
#endif
...
//here the virtual particles are connected so it is not necessary to add ghost control!
	...
	bondG[0] = collision_params.bond_vs;
        // Angular potential is defined on virtual particles, no need to ghost control? 
